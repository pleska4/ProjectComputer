...
CREATE TABLE IF NOT EXISTS `mydb`.`Payments` (
  `idPayment` INT NULL AUTO_INCREMENT,
  `sum` VARCHAR(45) NOT NULL,
  `discount` VARCHAR(45) NULL,
  `сustomers_id` INT NOT NULL,
  PRIMARY KEY (`idPayment`),
  INDEX `fk_Payments_Customers1_idx` (`сustomers_id` ASC, `idPayment` ASC) VISIBLE,
  CONSTRAINT `fk_Payments_Customers1`
    FOREIGN KEY (`сustomers_id` , `idPayment`)
    REFERENCES `mydb`.`Customers` (`id customer` , `id customer`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
